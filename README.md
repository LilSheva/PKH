...
3. **THOUGHT_MODE — три режима обработки размышлений модели:**
   ...
   - `SMART` — сейчас работает как `OFF` (thoughts не идут в базу, чтобы не засорять эмбеддинги и контекст). В будущем может использовать thoughts для обогащения метаданных блока, не вклеивая их в текст.
...
### `main_prompt` vs `meta_query`

Это два разных параметра в `generate_super_prompt()` и `save_super_prompt()`, и понимать разницу важно:

| Параметр | Что это | Куда идёт |
|----------|---------|-----------|
| `main_prompt` | **Задача для LLM.** Подробная формулировка того, что ты хочешь получить в ответ. | В `## TASK` секцию финального промпта, отправляется в LLM |
| `meta_query` | **Поисковый запрос по базе.** Короткий набор ключевых слов для поиска релевантного контекста в ChromaDB. | Используется только внутри PKH для векторного поиска, в LLM не уходит |

**Зачем разделять?** Формулировка задачи и слова для поиска часто разные. Пример:

```python
sniper.save_super_prompt(
    main_prompt="Напиши production-ready класс для обработки ошибок REST API на FastAPI с логированием в Sentry",
    meta_query="FastAPI middleware обработка исключений Sentry",
)
```

- `main_prompt` содержит слова "напиши", "мне", "production-ready" — если искать по ним, получишь шум из всех чатов где ты что-то просил написать.
- `meta_query` содержит только суть темы — PKH идёт точно в твои прошлые обсуждения FastAPI.

**Если `meta_query=None`** — PKH использует `main_prompt` как запрос. Это нормально для коротких формулировок, но для сложных задач лучше задавать оба.

### DialogBlock

Атомарная единица хранения в базе: одна пара «вопрос пользователя + ответ модели» из дампа чата.
...
# Быстрый поиск фрагментов по теме
hits = sniper.search_context("как я настраивал ChromaDB на Colab")

# Сборка супер-промпта: main_prompt = задача для LLM, meta_query = ключевые слова для поиска
prompt = sniper.generate_super_prompt(
    main_prompt="Напиши обёртку над ингестом, которая логирует прогресс по файлам",
    meta_query="ингест прогресс логирование",
)

# Сохранить промпт как .md файл (имя: HHMM-DDMMYY_main_slug_meta_slug_prompt.md)
path = sniper.save_super_prompt(
    main_prompt="Напиши обёртку над ингестом, которая логирует прогресс по файлам",
    meta_query="ингест прогресс логирование",
)
print(path)  # /content/drive/MyDrive/pkh_chroma/prompts/1435-110526_напиши-обёртку-над-инг_ингест-про_prompt.md
...
# Сброс базы (удалит коллекцию и манифест)
sniper.reset()
```
...
Фичи следующих этапов — после стабилизации ядра снайпера:

- **PKH Radar:** Визуализация кластеров смыслов базы (2D/3D scatter plot). UMAP + HDBSCAN поверх эмбеддингов из Chroma.
- **Unfinished Business / Persona Snapshot:** LLM-аналитика намерений и задач по базе — «над чем зависал», «что забросил», «какие паттерны повторяются».
- **Serendipity Widget:** Рандомизатор инсайтов на пересечении случайных тегов/кластеров.

## Статус

**Текущая фаза:** Этап 1 завершён. Первый прогон на полном архиве (2528 файлов → 660 чатов → 3564 блока) выполнен за ~50 минут на T4 GPU.
